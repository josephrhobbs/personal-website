...
\[ \begin{align*} P_2: \min_x 1/x & \\ \text{subject to } x &\ge 1 \end{align*} . \]
...
Hardness of NLP.  __Proof of Theorem 1__.  By Lemma 2, any SAT problem can be reduced to an NLP in \( O(N + kn) \) time.  Therefore, a polynomial-time reduction \( L_1 \) exists from SAT to NLP.  By Lemma 1, SAT is NP-hard, which means there exists a polynomial-time reduction \( L_2 \) exists from any problem in NP to SAT.  Therefore, the reduction \( L_3 := L_1(L_2(\cdot)) \) is polynomial-time.  This means that \( L_3 \) is a polynomial time reduction from any problem in NP to NLP.  By Definition 1, NLP is NP-hard \( \blacksquare \).
